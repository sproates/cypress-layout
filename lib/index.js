...
Cypress.Commands.add('isBelow', { prevSubject: true }, (subject, element = err.undefinedElement(), length = err.undefinedLength()) => comp.isPositioned(subject, 'below', element, length).then((subj) => {
  expect(subj[0], subj[1]).to.equal(true);
  return subject;
}));
